...
macro(DISSECT_VERSION)
    ...
    string(REGEX REPLACE "^([0-9]+).*" "\\1"
        FACEKIT_MAJOR_VERSION "${FACEKIT_VERSION}")
    string(REGEX REPLACE "^[0-9]+\\.([0-9]+).*" "\\1"
        FACEKIT_MINOR_VERSION "${FACEKIT_VERSION}")
    string(REGEX REPLACE "^[0-9]+\\.[0-9]+\\.([0-9]+).*" "\\1"
        FACEKIT_REVISION_VERSION "${FACEKIT_VERSION}")
    string(REGEX REPLACE "^[0-9]+\\.[0-9]+\\.[0-9]+(.*)" "\\1"
        FACEKIT_CANDIDATE_VERSION "${FACEKIT_VERSION}")
  ...
  if (NOT DEFINED LIB_INSTALL_DIR)
    ...
    if(NOT ANDROID)
      set(INCLUDE_INSTALL_ROOT
          "include/${PROJECT_NAME_LOWER}-${FACEKIT_MAJOR_VERSION}.${FACEKIT_MINOR_VERSION}")
    else(NOT ANDROID)
      set(INCLUDE_INSTALL_ROOT "include") # Android, don't put into subdir
    endif(NOT ANDROID)
    set(INCLUDE_INSTALL_DIR "${INCLUDE_INSTALL_ROOT}/pcl")
    set(DOC_INSTALL_DIR "share/doc/${PROJECT_NAME_LOWER}-${FACEKIT_MAJOR_VERSION}.${FACEKIT_MINOR_VERSION}")
    set(BIN_INSTALL_DIR "bin")
    set(PKGCFG_INSTALL_DIR "${LIB_INSTALL_DIR}/pkgconfig")
    if(WIN32 AND NOT MINGW)
        set(FACEKITCONFIG_INSTALL_DIR "cmake")
      else(WIN32 AND NOT MINGW)
        set(FACEKITCONFIG_INSTALL_DIR "share/${PROJECT_NAME_LOWER}-${FACEKIT_MAJOR_VERSION}.${FACEKIT_MINOR_VERSION}")
...
set(FACEKIT_KNOWN_GPU_ARCHS "20 21(20) 30 35 50")

###############################################################################
# A function for automatic detection of GPUs installed  (if autodetection is enabled)
# _detected_gpu   Variable set with output of the detection
# Usage:
#   FACEKIT_DETECT_GPUS(_detected_gpu)
function(FACEKIT_DETECT_GPUS _detected_gpu)
  # Is there some detection ?
  if(NOT CUDA_GPU_DETECT_OUTPUT)
    set(__cufile ${FACEKIT_BINARY_DIR}/detect_cuda_archs.cu)
  ...
  if(NOT CUDA_GPU_DETECT_OUTPUT)
    ...
    set(${_detected_gpu} ${FACEKIT_KNOWN_GPU_ARCHS} PARENT_SCOPE)
...
# Usage:
#   FACEKIT_SELECT_NVCC_ARCH_FLAGS(_nvcc_flags)
function(FACEKIT_SELECT_NVCC_ARCH_FLAGS _nvcc_flags)
  # List of arch names
  ...
  if(${CUDA_ARCH_NAME} STREQUAL "Manual")
    set(CUDA_ARCH_BIN ${FACEKIT_KNOWN_GPU_ARCHS} CACHE STRING "Specify 'real' GPU architectures to build binaries for, BIN(PTX) format is supported")
  ...
  elseif(${CUDA_ARCH_NAME} STREQUAL "All")
    set(__cuda_arch_bin ${FACEKIT_KNOWN_GPU_ARCHS})
  elseif(${CUDA_ARCH_NAME} STREQUAL "Auto")
    FACEKIT_DETECT_GPUS(__cuda_arch_bin)
